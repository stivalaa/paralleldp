...
#
# $Id: runall_httslf.sh 3160 2009-12-29 02:22:18Z alexs $
# 
###############################################################################
...
while [ $numthreads -ge 1 ]; do
  ...
  while [ $iter -le $MAXITERATIONS ]; do
      /usr/bin/time $TIMEGUARD $TIMEOUT run_instances.py  knapsack_httslf "-r${numthreads} ${knapsack_options}" instances > ${outdir}/instances_httslf_${numthreads}_${iter}.out 2> ${outdir}/instances_httslf_${numthreads}_${iter}.err
      iter=`expr $iter + 1`
  done
